...
fn test_leader_to_validator_transition() {
    ...
    let (mint, leader_ledger_path, genesis_entries) = create_sample_ledger(
...
fn test_leader_validator_basic() {
    ...
    let (mint, leader_ledger_path, genesis_entries) =
        create_sample_ledger("test_leader_validator_basic", 10_000, num_ending_ticks);
...
fn test_dropped_handoff_recovery() {
    ...
    let (mint, bootstrap_leader_ledger_path, genesis_entries) =
        create_sample_ledger("test_dropped_handoff_recovery", 10_000, num_ending_ticks);
