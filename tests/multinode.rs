...
fn test_leader_to_validator_transition() {
    ...
    let random_id = Keypair::new().pubkey();

    // Set the next leader to be some random id not equal to the leader
    leader.set_scheduled_leader(random_id, LEADER_ROTATION_INTERVAL);
    ...
    // Generate LEADER_ROTATION_INTERVAL transactions
    ...
    // Query leader to make sure that he has the proper balances
    let mut leader_client = mk_client(&leader_info);

    let expected_bal = LEADER_ROTATION_INTERVAL - entries.len() as u64;
