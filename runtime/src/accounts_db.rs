...
impl AccountsDB {
    ...
    fn store_accounts_default(
        ...
        self.store_accounts_custom(
            ...
            accounts,
            hashes,
            None::<Box<dyn FnMut(Slot) -> Arc<AccountStorageEntry>>>,
    ...
    fn store_accounts_custom(
        ...
        accounts: &[(&Pubkey, &Account)],
        hashes: &[Hash],
        storage_finder: Option<Box<dyn FnMut(Slot) -> Arc<AccountStorageEntry>>>,
        write_version_producer: Option<Box<dyn Iterator<Item = u64>>>,
    ) -> StoreAccountsTiming {
        let storage_finder: Box<dyn FnMut(Slot) -> Arc<AccountStorageEntry>> =
            if let Some(storage_finder) = storage_finder {
                storage_finder
            } else {
                Box::new(|slot| self.find_storage_candidate(slot))
            };

        let write_version_producer: Box<dyn Iterator<Item = u64>> =
            if let Some(write_version_producer) = write_version_producer {
