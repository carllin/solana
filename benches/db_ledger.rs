...
use solana::packet::{Blob, SharedBlob, BLOB_HEADER_SIZE};
use test::Bencher;
    ...
    bench.iter(move || {
        ...
        for i in start_index..start_index + num_reads {
            let _ =
                db_ledger
                    .data_cf
                    .get_on_slot_index(&db_ledger.db, slot, i as u64 % total_blobs);
        }
    });
...
fn bench_read_random(bench: &mut Bencher) {
    ...
    bench.iter(move || {
        for i in indexes.iter() {
            let _ = db_ledger
                .data_cf
                .get_on_slot_index(&db_ledger.db, slot, *i as u64);
        }
    });
