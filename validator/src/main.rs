...
fn start_gossip_node(
    ...
    let gossip_service =
        GossipService::new(&cluster_info, None, gossip_socket, None, &gossip_exit_flag);
...
fn download_then_check_genesis_hash(
    ...
    let genesis_config = if let Ok(tmp_genesis_package) =
        download_genesis_if_missing(rpc_addr, &genesis_package, false)
    {
...
pub fn main() {
    ...
    let validator = Validator::new(
        votes_file,
        Node::new_localhost_with_pubkey(&pubkey),
        &identity_keypair,
        &ledger_path,
        &Pubkey::default(),
        vec![],
