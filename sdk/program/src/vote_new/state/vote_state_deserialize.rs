use {
    crate::{
        clock::Epoch, instruction::InstructionError, pubkey::Pubkey, serialize_utils::cursor::*,
        vote_new::state::*,
    },
...
fn read_vote_range<T: AsRef<[u8]>>(
    cursor: &mut Cursor<T>,
    has_latency: bool,
) -> Result<VoteRange, InstructionError> {
    let slot = read_u64(cursor)?;
    let reference_slot = read_u64(cursor)?;
