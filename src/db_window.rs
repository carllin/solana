...
pub fn repair(
    ...
    info!(
        "REPAIR: {}, consumed: {}, MREH: {}, IDXS: {:?}",
        id, consumed, max_repair_entry_height, idxs
    );
...
pub fn process_blob(
    ...
    if blob.read().unwrap().slot().is_err() {
        info!("NO SLOT: Processing blob with index: {}", pix);
    }
    let slot = blob.read().unwrap().slot()?;
    let leader = leader_scheduler.get_leader_for_slot(slot);
    info!("PROCESSING BLOB WITH ID: {} WITH SLOT: {}", pix, slot);
