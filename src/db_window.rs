...
pub fn repair(
    ...
    println!(
        "REPAIR: {}, MREH: {}, IDXS: {:?}",
        id, max_repair_entry_height, idxs
    );
...
pub fn process_blob(
    ...
    if blob.read().unwrap().slot().is_err() {
        println!("NO SLOT: Processing blob with index: {}", pix);
    }
    let slot = blob.read().unwrap().slot()?;
    let leader = leader_scheduler.get_leader_for_slot(slot);
    println!("PROCESSING BLOB WITH ID: {} WITH SLOT: {}", pix, slot);
