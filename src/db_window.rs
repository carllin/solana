    ...
    let data_blobs: Vec<_> = blobs
        ...
        .filter(|blob| {
            ...
            let (slot, pix, size) = {
                ...
                (r_blob.slot(), r_blob.index(), r_blob.size())
            };

            if slot.is_err() || pix.is_err() || size.is_err() {
                println!("Received invalid blob for processing");
                return false;
            }

            let (slot, pix, size) = { (slot.unwrap(), pix.unwrap(), size.unwrap()) };
            ...
            if is_coding {
                ...
                let _ = db_ledger.erasure_cf.put(
                    &db_ledger.db,
                    &erasure_key,
                    &rblob.data[..BLOB_HEADER_SIZE + size],
                );
            }
            // We only want the data blobs in the filter
            !is_coding
        })
        .collect();

    let mut consumed_entries = db_ledger.write_shared_blobs(data_blobs)?;
