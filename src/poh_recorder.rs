...
impl PohRecorder {
    ...
    pub fn hash(&self) -> Result<()> {
        ...
        if self.check_max_tick_height_reached() {
    ...
    pub fn tick(&mut self) -> Result<u64> {
        ...
        if self.check_max_tick_height_reached() {
            ...
            println!("CREATING TICK WITH ID: {}, height: {}", poh.last_hash, self.tick_height);
            Ok(self.tick_height)
        }
    }
    ...
    fn check_max_tick_height_reached(&self) -> bool {
        if let Some(max_tick_height) = self.max_tick_height {
            if self.tick_height < max_tick_height {
                false
            } else {
                true
            }
