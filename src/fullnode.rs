...
mod tests {
    ...
    fn test_validator_to_leader_transition() {
        ...
        let t_responder = {
            ...
            let mut num_blobs_to_make = total_blobs_to_send as usize;
            ...
            while num_blobs_to_make != 0 {
                ...
                let mut new_blobs: SharedBlobs = new_entries
                    ...
                    .enumerate()
                    .map(|(i, e)| {
                        let blob_index = (num_blobs_to_make - i - 1) as u64;
                        let new_blob = e.to_blob(
                            &resp_recycler,
                            Some(blob_index),
                            Some(leader_id),
                            Some(tvu_address),
                ...
                new_blobs.truncate(num_blobs_to_make);
                num_blobs_to_make -= new_blobs.len();
