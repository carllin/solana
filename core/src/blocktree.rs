...
#[cfg(not(feature = "kvstore"))]
use self::rocks::{DataCf, DetachedHeadsCf, ErasureCf, MetaCf, Rocks};
...
impl Blocktree {
    ...
    fn handle_chaining_for_slot(
...

        {
            ...
            if is_detached_head {
                write_batch
                    .delete_cf(self.detached_heads_cf.handle(), &DetachedHeadsCf::key(slot))?;
            }
        }
...
pub mod tests {
    ...
    fn get_detached_heads(blocktree: &Blocktree) -> Vec<u64> {
        ...
        iter.seek_to_first();
        while iter.valid() {
            results.push(DetachedHeadsCf::index_from_key(&iter.key().unwrap()).unwrap());
            iter.next();
        }
        results
    }
