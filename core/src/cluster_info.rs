...
impl ClusterInfo {
    ...
    fn handle_pull_requests(
        ...
        let now = timestamp();
        {
            ...
            // allow 10kBps per staked validator, epoch slots + votes ~= 1.5kB/slot ~= 4kB/s
            const BYTES_PER_INTERVAL: usize = 1000;
        ...
        let mut stats: Vec<_> = pull_responses
            ...
            .map(|(i, (responses, _from_addr))| {
                let score: u64 = if let Some(stake) = stakes.get(&responses[0].pubkey()) {
                    *stake
                } else {
                    1
                };
