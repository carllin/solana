            ...
            .spawn(move || {
                ...
                loop {
                    ...
                    for slot in newly_computed_slot_stats {
                        ...
                        Self::mark_slots_confirmed(&my_pubkey, &confirmed_forks, &blockstore, &bank_forks, &mut progress, &mut duplicate_slots_tracker, &mut heaviest_subtree_fork_choice,  &mut epoch_slots_frozen_slots, &mut duplicate_slots_to_repair);
                    }
        ...
        for maybe_purgeable_duplicate_slots in epoch_slots_frozen_receiver.try_iter() {
                ...
                maybe_purgeable_duplicate_slots.into_iter()
            {
                ...
                check_slot_agrees_with_cluster(
                    ...
                    SlotStateUpdate::EpochSlotsFrozen(epoch_slots_frozen_hash),
                );
            }
        }
    }
...
pub mod tests {
    ...
    fn setup_vote_then_rollback(
        ...
        */
        ...
        let mut duplicate_slots_tracker = DuplicateSlotsTracker::default();
        let mut duplicate_slots_to_repair = DuplicateSlotsToRepair::default();
        let mut eppoch_slots_frozen_slots = EpochSlotsFrozenSlots::default();
    ...
    pub(crate) type GenerateVotes = Box<dyn Fn(Vec<Pubkey>) -> HashMap<Pubkey, Vec<Slot>>>;

    pub(crate) fn setup_forks_from_tree(
        tree: Tree<Slot>,
        num_keys: usize,
        generate_votes: Option<GenerateVotes>,
