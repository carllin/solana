...
impl ReplayStage {
    ...
    fn process_duplicate_slots(
        ...
        progress: &mut ProgressMap,
        blockstore: &Blockstore,
        fork_choice: &mut dyn ForkChoice,
    ) {
        for duplicate_slot in duplicate_slots_receiver.try_iter() {
            Self::process_duplicate_slot(
                duplicate_slot,
                descendants,
                progress,
                blockstore,
        ...
        if blockstore.is_root(duplicate_slot) {
            ...
            if progress.get(&duplicate_slot).is_some() {
                if !blockstore.is_unconfirmed_duplicate(duplicate_slot) {
                    ...
                    fork_choice.mark_slot_invalid_candidate(duplicate_slot);
    ...
    pub(crate) fn select_vote_and_reset_forks(
        ...
        let selected_fork = {
            ...
            if let SwitchForkDecision::FailedSwitchThreshold(_, _) = switch_fork_decision {
                // The heaviest fork is never descended from a known unconfirmed duplicate slot
                // because the fork choice rule ensures that. Thus the only time we have to worry
                // about duplicate slots here is if we can't vote on the heaviest fork, and are
                // considering resetting to the `heaviest_bank_on_same_voted_fork`.
                let latest_duplicate_ancestor = heaviest_bank_on_same_voted_fork
                    .as_ref()
                    .map(|heaviest_bank_on_same_voted_fork| {
                        progress.latest_unconfirmed_duplicate_ancestor(
                            heaviest_bank_on_same_voted_fork.slot(),
                        )
                    })
                    .unwrap_or(None);
                let is_last_vote_ancestor_unconfirmed_duplicate =
                    latest_duplicate_ancestor.is_some();

                let reset_bank = if is_last_vote_ancestor_unconfirmed_duplicate {
                    Some(heaviest_bank)
                } else {
                    heaviest_bank_on_same_voted_fork
                };

                // If we can't switch, then
                // a) If our last vote was on a non-duplicate/confirmed slot, then
                // reset to the the next votable bank on the same fork as our last vote,
                // but don't vote
                // b) If our last vote was on an unconfirmed, duplicate slot, then we need
    ...
    fn mark_slots_confirmed(
        ...
        fork_choice.mark_slots_valid_candidate(confirmed_forks);
    }
...
pub(crate) mod tests {
    ...
    fn test_unconfirmed_duplicate_slots_not_votable() {
        ...
        ReplayStage::process_duplicate_slot(
            2,
            &descendants,
            &mut progress,
            &blockstore,
