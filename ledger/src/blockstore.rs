...
impl Blockstore {
    ...
    pub fn add_completed_slot_sender(&mut self) -> CompletedSlotsReceiver {
        let (completed_slot_sender, completed_slot_receiver) =
            sync_channel(MAX_COMPLETED_SLOTS_IN_CHANNEL);
        self.completed_slots_senders.push(completed_slot_sender);
        completed_slot_receiver
    }
    ...
    fn should_insert_data_shred(
        ...
        if shred_index >= last_index {
            info!("got shred index greater than last in slot");
        ...
        if last_in_slot && shred_index < slot_meta.received {
            info!("got last in slot less than received");
            let leader_pubkey = leader_schedule
                .map(|leader_schedule| leader_schedule.slot_leader_at(slot, None))
                .unwrap_or(None);
