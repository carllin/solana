...
impl LocalCluster {
    ...
    fn do_add_validator(
        ...
        let client = self
            .build_tpu_quic_client(&self.entry_point_info)
            .expect("tpu_client");
...
impl Cluster for LocalCluster {
    ...
    fn get_validator_client(&self, pubkey: &Pubkey) -> Option<QuicTpuClient> {
        self.validators.get(pubkey).map(|validator_info| {
            self.build_tpu_quic_client(&validator_info.info.contact_info)
                .expect("should build tpu quic client")
        })
    }

    fn build_tpu_quic_client(&self, contact_info: &ContactInfo) -> Result<QuicTpuClient> {
